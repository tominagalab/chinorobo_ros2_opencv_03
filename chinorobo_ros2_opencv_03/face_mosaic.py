...
# ノード名
NODE_NAME = 'face_mosaic_node'  # 必要に応じて変更すること
...
def image_proc(src):
    ...
    for (x, y, w, h) in faces:
        # 顔領域を切り出し
        face_roi = dst[y:y+h, x:x+w]
        # モザイク処理（リサイズして元に戻す）
        small = cv2.resize(face_roi, (9, 9), interpolation=cv2.INTER_LINEAR)
        mosaic_face = cv2.resize(small, (w, h), interpolation=cv2.INTER_NEAREST)
        # 元の画像にモザイク顔を貼り付け
        dst[y:y+h, x:x+w] = mosaic_face
